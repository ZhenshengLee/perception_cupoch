...
#include <benchmarks/rmm/utilities/cxxopts.hpp>
...
inline auto make_managed_arena()
{
  return rmm::mr::make_owning_wrapper<rmm::mr::arena_memory_resource>(make_managed());
}
...
void declare_benchmark(std::string name)
{
  ...
  if (name == "cuda_async")
    ...
    BENCHMARK_CAPTURE(BM_RandomAllocations, managed_mr, &make_managed)->Apply(benchmark_range);
  else if (name == "managed_pool")
    BENCHMARK_CAPTURE(BM_RandomAllocations, managed_pool_mr, &make_managed_pool)->Apply(benchmark_range);
  else if (name == "arena")
    BENCHMARK_CAPTURE(BM_RandomAllocations, arena_mr, &make_arena)->Apply(benchmark_range);
  else if (name == "managed_arena")
    BENCHMARK_CAPTURE(BM_RandomAllocations, managed_arena_mr, &make_managed_arena)->Apply(benchmark_range);
  else
...
#ifdef RMM_CUDA_MALLOC_ASYNC_SUPPORT
      std::array<std::string, 8> mrs{"pool", "binning", "arena", "cuda_async", "cuda", "managed", "managed_pool", "managed_arena"};
#else
      std::array<std::string, 7> mrs{"pool", "binning", "arena", "cuda", "managed", "managed_pool", "managed_arena"};
#endif
